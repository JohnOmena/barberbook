...
# executa tudo num psql
$cmd = @("docker","compose","exec","-T","postgres","psql","-U","barber","-d","barberbook","-v","ON_ERROR_STOP=1","-c",$sql)
...
$proc.WaitForExit()
if ($proc.ExitCode -eq 0) { Write-Host "Seed OK." -ForegroundColor Green } else { Write-Host "Seed falhou." -ForegroundColor Red }

# Listagens rápidas (atenção à sensibilidade de maiúsculas)
docker compose exec -T postgres psql -U barber -d barberbook -c 'SELECT "Id","Name" FROM tenants;'
docker compose exec -T postgres psql -U barber -d barberbook -c 'SELECT "Name","IsDefault" FROM professionals;'
docker compose exec -T postgres psql -U barber -d barberbook -c 'SELECT "Name","DurationMin" FROM services ORDER BY "Name";'
docker compose exec -T postgres psql -U barber -d barberbook -c 'SELECT "Weekday","Start","End" FROM availabilities ORDER BY 1,2;'
